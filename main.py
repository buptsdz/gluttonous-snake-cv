...
def update_game():
    ...
    if update_flag:
        ...
        # 检测手势
        lm_list = find_hands(img)
        if lm_list:
            pointindex = lm_list[8]
...
def countdown():
    ...
    if countdown_flag:
        ...
        if not lm_list:
            ...
            cv2.putText(
                img1, text, (text_x, text_y), font, font_scale, (255, 255, 255), 2
            )
        else:
            if start_time == 0:
            ...
            if 0 <= elapsed_time <= 1.5:
                img1 = game.show3(img1, pointindex)
            elif 1.5 < elapsed_time <= 2.8:
        ...
        if start_time == 0 or (time.time() - start_time) <= 4.1:
...
def show_score(score):
    ...
    score_label = tk.Label(
        score_window, text=f"你的得分: {score}", font=("Helvetica", 14)
    )
    ...
    if score <= history:
        ...
        history_label = tk.Label(
            score_window, text=f"历史最高: {history}", font=("Helvetica", 12)
        )
        ...
        history_label = tk.Label(
            score_window, text=f"恭喜你！成为历史最高: {score}", font=("Helvetica", 12)
        )
...
def show_start_screen():
    ...
    start_button = tk.Button(
        window, text="Start Game", command=start_game, relief=tk.RAISED, borderwidth=5
    )
    ...
    # 历史得分
    history_label = tk.Label(
        window, text=f"历史最高: {history}", font=("Helvetica", 20), height=1, width=10
    )
    history_label.place(relx=0.3, rely=0.7, anchor="center")

    # 清空历史按钮
    clear_history_button = tk.Button(
        window, text="Clear History", command=clear_history_callback
    )
...
def end_game():
    ...
    score_label = tk.Label(
        window, text=f"Final Score: {game.score}", font=("Arial", 18)
    )
